...
public class TicTacToeGame implements ActionListener {
    ...
    JLabel textfield = new JLabel(); // Se declaran todas las funciones
    ...
    TicTacToeGame() {
        ...
        frame.getContentPane().setBackground(new Color(50, 50, 50)); // Constantes de todo JFrame
        frame.setTitle("Tic Tac Toe Paulinilla");
        frame.setLayout(new BorderLayout()); // Le ponemos un border layout
        frame.setResizable(false);
        frame.setVisible(true);

        textfield.setBackground(new Color(189, 183, 107));
        textfield.setForeground(new Color(25, 255, 0));
        textfield.setFont(new Font("Ink Free", Font.BOLD, 75)); // Texto que muestra los resultados
        ...
        btn_panel.setLayout(new GridLayout(3, 3)); // Este es el grid donde van a ir todos los botones
        btn_panel.setBackground(new Color(150, 150, 150));

        for (int i = 0; i < 9; i++) {
            bton[i] = new JButton();
            btn_panel.add(bton[i]);
            bton[i].setFont(new Font("Ink Free", Font.BOLD, 120)); // Aca creamos los 9 botones para jugar "gato"
            bton[i].setFocusable(false);
            bton[i].addActionListener(this);
        }

        title_panel.add(textfield); // Aca le metemos el label text_field al panel de titulo
    ...
    public void startGame() {
        try {
            ...
            Thread.sleep(1000); // Esperamos alguna excepcion
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        int chance = random.nextInt(100);              //Genera una ramdom

        if (chance % 2 == 0) {
            pl1_chance = true;
            textfield.setText("X turn");                //Esto se utiliza para que siempre inicie uno diferente
        } else {
            pl1_chance = false;
